...
# Ticket KCP-085 — Add RenderPackStatus node (what’s missing?)

Status: DONE

Goal (1 sentence): Provide a read-only status node that reports total set items, saved-media count, and missing idx list.

Scope:

Must include:
- Node file `kcp/nodes/render_pack_status.py` with class `KCP_RenderPackStatus`.
- Required inputs: `db_path`, `set_id`, `strict`.
- Returns `summary_text`, `status_json` with machine-readable keys including `total_items`, `items_with_media`, `missing_idxs`.
- `missing_idxs` sorted ascending.
- strict=True raises `kcp_set_media_missing: set_id=<...> missing_idxs=[...]`.
- Verify smoke for counts/sorted missing behavior.

Must NOT include:
- No schema changes.

Acceptance criteria:
- Status output is consistent with DB rows + file existence.
- strict mode errors when missing media exists.
- `python tools/verify.py --receipt "KCP-085"` passes.

Evidence needed (what to paste/verify):
- `python tools/verify.py --receipt "KCP-085"`

Notes / implementation hints:
- Reuses canonical root resolution from db_path.

Risks:
- Low.

# Ticket KCP-086 — Add KeyframeSetSummary node (human-friendly)

Status: DONE

Goal (1 sentence): Provide a human-readable set summary including provenance and picked/index counts for quick sanity checks.

Scope:

Must include:
- Node file `kcp/nodes/keyframe_set_summary.py` with class `KCP_KeyframeSetSummary`.
- Required inputs: `db_path`, `set_id`.
- Return `summary_text`, `status_json`.
- `summary_text` includes at least: `set_id`, `stack_id`, `variant_policy_id`, `item_count`, `picked_index`.
- `status_json` includes at least: `set_id`, `total_items`, `picked_index`, `variant_policy_id`, `stack_id`.

Must NOT include:
- No schema changes.

Acceptance criteria:
- Valid set returns summary including picked_index and total_items/provenance.
- `python tools/verify.py --receipt "KCP-086"` passes.

Evidence needed (what to paste/verify):
- `python tools/verify.py --receipt "KCP-086"`

Notes / implementation hints:
- Uses DB counts rather than trusting inputs.

Risks:
- Low.

# Ticket KCP-087 — Expose batch_index in SetItemSaveImage UI

Status: DONE

Goal (1 sentence): Make `batch_index` visible in `KCP_KeyframeSetItemSaveImage` so users can select which render-batch image to save.

Scope:

Must include:
- `batch_index` appears in `INPUT_TYPES` as INT default 0 min 0.
- Out-of-bounds raises `kcp_batch_index_oob: batch_index=<...> batch_size=<...>`.
- Verify smoke asserts input presence, OOB error, and selection behavior.

Must NOT include:
- No schema changes.

Acceptance criteria:
- UI-exposed `batch_index` works and OOB does not silently save wrong image.
- `python tools/verify.py --receipt "KCP-087"` passes.

Evidence needed (what to paste/verify):
- `python tools/verify.py --receipt "KCP-087"`

Notes / implementation hints:
- Existing save semantics and DB/file layout remain unchanged.

Risks:
- Low/medium.

# Ticket KCP-088 — Add SetItemSaveBatch node (save the whole pack)

Status: DONE

Goal (1 sentence): Persist an IMAGE batch into sequential set-item media slots in one node call.

Scope:

Must include:
- Node file `kcp/nodes/keyframe_set_item_save_batch.py` with class `KCP_KeyframeSetItemSaveBatch`.
- Required inputs: `db_path`, `set_id`, `idx_start`, `images`, `format`, `overwrite`.
- Return values: `set_id`, `saved_count`, `batch_json`.
- Batch save loop writes image+thumb per idx and updates DB media paths.
- Missing row fails fast with `kcp_set_item_not_found` + first missing idx diagnostics.
- Verify smoke asserts N rows updated and N files/thumbs exist.

Must NOT include:
- No schema changes.

Acceptance criteria:
- Batch size N persists N media items and DB updates.
- Missing idx errors with actionable diagnostics.
- `python tools/verify.py --receipt "KCP-088"` passes.

Evidence needed (what to paste/verify):
- `python tools/verify.py --receipt "KCP-088"`

Notes / implementation hints:
- Uses existing atomic image write + thumbnail helpers.

Risks:
- Medium.

# Ticket KCP-089 — Add VariantUnroll node (list-mapping for ComfyUI)

Status: DONE

Goal (1 sentence): Unroll `variant_list_json` into aligned list outputs for list-mapped KSampler execution.

Scope:

Must include:
- Node file `kcp/nodes/variant_unroll.py` with class `KCP_VariantUnroll`.
- Input: `variant_list_json` (multiline STRING).
- List outputs for idx/prompt/params including `item_json`-equivalent payload via `gen_params_json_list`.
- `OUTPUT_IS_LIST` enabled for all outputs.
- Verify smoke checks output lengths/values and stable ordering.
...
Acceptance criteria:
- Output lists are aligned and deterministic for input variants.
- `python tools/verify.py --receipt "KCP-089"` passes.

Evidence needed (what to paste/verify):
- `python tools/verify.py --receipt "KCP-089"`

Notes / implementation hints:
- Typed sampler/scheduler outputs remain wiring-compatible.

Risks:
- Medium-low.

# Ticket KCP-090 — Opinionated on-ramp workflow (render → persist → pick winner)

Status: DONE

Goal (1 sentence): Document a clear end-to-end on-ramp wiring and ship a reference workflow JSON to reduce misuse.

Scope:

Must include:
- README section `Opinionated On-Ramp: Render → Persist → Pick Winner`.
- Explicit wiring lines for VariantPack→VariantUnroll, list-mapped KSampler inputs, SaveBatch persistence, and winner loop pick/load/item-pick/mark/promote chain.
- New workflow file `examples/workflows/opinionated_onramp_render_persist_pick_winner.json`.
- Verify smoke checks README on-ramp header + wiring strings.
...
Acceptance criteria:
- Workflow file exists and includes render-persist-pick-winner nodes.
- README contains winner-loop and on-ramp wiring references.
- `python tools/verify.py --receipt "KCP-090"` passes.

Evidence needed (what to paste/verify):
- `python tools/verify.py --receipt "KCP-090"`

Notes / implementation hints:
- Workflow is a minimal wiring guide, not a model-dependent runnable fixture.
